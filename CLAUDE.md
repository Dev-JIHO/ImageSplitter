...
`*.tsx` 컴포넌트/훅 테스트는 `@testing-library/react`를 사용하며 `src/test/setup.ts`(jest-dom 매처)가 `vite.config.ts`의 `test.setupFiles`로 연결되어 있다. `.github/workflows/ci.yml`에서 push·PR마다 lint·test·build를 실행한다.
...
핵심 원칙: **도메인 로직은 `src/lib/`의 순수 함수로, 파생 상태는 `src/hooks/`로, UI는 `src/controls`·`src/preview`·`src/components`로** 분리한다. `App.tsx`는 상태 보관 + 조립만 담당하는 얇은 컴포넌트다.

### 폴더 구조

| 위치 | 책임 |
|------|------|
| `App.tsx` | 상태(`settings` 등) 보관 + 훅 호출 + 패널·모달 조립. `SettingsProvider`로 settings를 하위에 공급 |
| `types.ts` / `constants.ts` | `Settings`·`LayoutState` 등 공유 타입, `initialSettings`·지원 이미지 형식 등 상수 |
| `SettingsContext.tsx` | `useSettings()`로 `{ settings, setSettings, updateSetting }` 접근 (prop drilling 제거) |
| `hooks/` | `usePreparedImage`, `usePosterLayout`, `usePrintScale`, `useImageUpload`, `useCanvasZoom`, `useCropDrag`, `usePasteImage`(Ctrl+V 붙여넣기), `useOnboarding`(첫 방문 투어 상태), `useModalA11y`(모달 Escape 닫기·포커스 트랩) |
| `controls/` | 좌측 설정 패널: `SettingsPanel` + 섹션(SeamTest/ImageUpload/SizingMode/FitAndOverlap/DisplayOptions/PrintOptions) + `Summary` |
| `preview/` | 우측 미리보기: `PreviewPanel`, `PreviewCanvas`, `PreviewSidebar`(줌·회전·내보내기·요약), `PreviewLegend`, `EmptyPreview`, 순수 그리기 `drawPoster.ts` |
| `components/` | 범용 컴포넌트: `NumberField`, `ExportConfirmModal`, `AdvancedHelpModal`, `Onboarding`, `MobileBottomNav`, `ErrorBoundary`, `Chevron`, `InfoHint`, `AdSlot`(콘텐츠 페이지 전용 광고 유닛, [docs/adsense-launch-todo.md](docs/adsense-launch-todo.md) 참고) |
| `lib/` | mm 단위 순수 도메인 로직 (아래 표) + `num.ts`(수치 유틸), `imageSource.ts`(이미지 회전), `adsenseConfig.ts`/`adsenseLoader.ts`(애드센스 설정·지연 로드) |
...
| `imageLoader.ts` | File → HTMLImageElement + objectURL. 파일 크기(50MB)·픽셀수(6,000만) 상한 검증, HEIC/HEIF는 `heic2any`로 동적 임포트해 JPEG로 변환 후 로드 |
...
- **페이지 번호 표시(showLabel)**: 번호는 이미지 밖 빈 공간(풀칠 탭 또는 fit 모드의 여백)에만 놓는다. 빈 공간이 없는 슬라이스는 번호를 표시하지 않는다.
